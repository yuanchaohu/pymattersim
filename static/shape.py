...
def gyration_tensor(pos_group: np.ndarray) -> List[Any]:
    ...
    if ndim==3:
        asphericity = 1.5*principal_component[2] - 0.5*principal_component.sum()
        shape_anisotropy  = (asphericity ** 2 + 0.75 * acylindricity ** 2) / radius_of_gyration**4
        logger.info(
            f"""
        ...
        return [radius_of_gyration, asphericity, acylindricity, shape_anisotropy, fractal_dimension]
    else:
        logger.info(
        f"""
        Calculated shape factors are:
