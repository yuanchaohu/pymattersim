...
class HessianMatrix:
    ...
    def diagonalize_hessian(
        ...
        Return:
            None
        """
        logger.info(f"Calculate and diagonalize hessian matrix of d={self.ndim} system")
        ...
        for i in range(self.epsilons.shape[0]):
            for j in range(self.epsilons.shape[1]):
                prefactor[i, j] = 1.0/np.sqrt(self.masses[i+1]*self.masses[j+1])

        #calcualte hessian matrix
        hessian_matrix = np.zeros((self.ndim*nparticle, self.ndim*nparticle))
        for i in range(nparticle):
            ...
            for j in range(nparticle):
                ...
                if (j != i) & (distance[j] <= self.r_cuts[itype, jtype]):
                    ...
                    #for i-i pair
                    hessian_matrix[index_i_0:index_i_1, index_i_0:index_i_1] += dudr2i * prefactor[itype, jtype]
                    #for i-j pair
        ...
        for i in range(evecs.shape[1]):
            PR[i] = participation_ratio(evecs[:, i].reshape(nparticle, self.ndim))
        frequencies = np.where(evals>0, np.sqrt(evals), evals)
        pd.DataFrame({"omega": frequencies, "PR": PR}).to_csv(outputfile+".omega_PR.csv", index=False)
