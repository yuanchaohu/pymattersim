...
class TestVector(unittest.TestCase):
    ...
    def test_participation_ratio(self) -> None:
        ...
        logger.info(f"Finishing test gyration_tensor using {self.test_file_2d}...")
    ...
    def test_local_vector_alignment(self) -> None:
        ...
        np.testing.assert_almost_equal(lva2[:10],np. array([ 0.00992724, -0.39541864, -0.06977997,  0.02699935, -0.05977384,
    ...
    def test_divergence_curl_2d(self) -> None:
        ...
        divergence1 = divergence_curl(self.input_vp.snapshots.snapshots[0],v1,ppp,neighborfile)[:10]
    ...
    def test_divergence_curl_3d(self) -> None:
        ...
        self.input_v.read_onefile()
        ppp = np.array([1,1,1])
        Nnearests(self.input_vp.snapshots, N=12, ppp=ppp, fnfile='neighborlist.dat')
        neighborfile='neighborlist.dat'
        v1 = self.input_v.snapshots.snapshots[0].positions
        
        divergence,curl = divergence_curl(self.input_vp.snapshots.snapshots[0],v1,ppp,neighborfile)[:10]
        
        np.testing.assert_almost_equal(divergence[:10],np.array([ 0.13229511,  0.06307015, -0.2782819 , -0.00341545, -0.40276813,
                                                                 -0.18070488,  0.38035838, -0.27656106, -0.13100627, -0.2772078 ]))
        np.testing.assert_almost_equal(curl[:10], np.array([[ 0.1027198 ,  0.05574753, -0.00101901],
    ...
    def test_vibrability(self) -> None:
        ...
        Test vibrability
        """
        logger.info(f"Starting test vibrability...")
        ...
        np.testing.assert_almost_equal(vb,np.array([0.42386879, 0.2696528 , 0.4320155 , 0.57338379, 1.14279844,
                                                    0.41539947, 0.36564011, 0.2999141 , 0.09689469, 0.18054036]))
        logger.info(f"Finishing test vibrability...")
    ...
    def test_vector_decomposition_sq(self) -> None:
        ...
        v1 = self.input_v.snapshots.snapshots[0].positions
        v2 = self.input_v.snapshots.snapshots[9].positions
        _, ave_sqresults1 = vector_decomposition_sq(self.input_vp.snapshots.snapshots[0],choosewavevector(2,10),v1)
        _, ave_sqresults2 = vector_decomposition_sq(self.input_vp.snapshots.snapshots[9],choosewavevector(2,10),v2)
        ...
        np.testing.assert_almost_equal(ave_sqresults1["Sq"].values,
                                       np.array([0.43436396, 0.71551347, 0.78594025, 0.12707948, 0.90674292]))
        ...
        np.testing.assert_almost_equal(ave_sqresults2["Sq"].values,
                                       np.array([1.74958771, 1.13272437, 0.66712606, 0.4084669 , 0.8715305 ]))
    ...
    def test_vector_fft_corr(self) -> None:
        ...
        v =[]
        for i in range(self.input_v.snapshots.nsnapshots):
            temp = self.input_v.snapshots.snapshots[i].positions
            v.append(temp)
        v=np.array(v)
        alldata = vector_fft_corr(self.input_vp.snapshots,qvector,v,outputfile="test")
        spectra = pd.read_csv("test.spectra.csv")
        T_fft = np.load("testT_FFT.npy")
        L_fft = np.load("testL_FFT.npy")
        ...
        np.testing.assert_almost_equal(T_fft[0,3:],
                                       np.array([ 1.        ,  0.33282675,  0.10380247,  0.06949692,  0.16854733,
                                                 0.46965734,  0.01265494, -0.18949574,  0.52572456,  0.17707337]))
        np.testing.assert_almost_equal(L_fft[0,3:],
                                       np.array([ 1.        , -0.08931861,  0.18748259,  0.14045109,  0.20976937,
                                                 -0.36043121,  0.04820545, -0.19739207, -1.01346368,  0.06020561]))

        os.remove("test.spectra.csv")
        os.remove("testT_FFT.npy")
        os.remove("testL_FFT.npy")
        os.remove("testFFT.npy")
