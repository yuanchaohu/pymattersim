...
class Test_Coarse_Graining(unittest.TestCase):
    ...
    def test_spatial_average(self) -> None:
        ...
        """
        logger.info(
            f"Starting test spatial_average using {self.test_file_2d}...")
        readdump = DumpReader(self.test_file_2d, ndim=2)
        readdump.read_onefile()

        Nnearests(readdump.snapshots, N=6, ppp=np.array(
        ...
        np.testing.assert_almost_equal(
            np.array([
                0.57277224, 0.53034253, 0.63984439, 0.49477918, 0.75474255,
                0.7225598, 0.59054183, 0.54235565, 0.59980761, 0.5684533
            ]), sa[:, 0]
        )
    ...
    def test_gaussian_blurring_2d(self) -> None:
        ...
        """
        logger.info(
            f"Starting test gaussian_blurring using {self.test_file_2d}...")
        readdump = DumpReader(self.test_file_2d, ndim=2)
        readdump.read_onefile()
        ppp = ppp = np.array([1, 1])
        ngrids = [20, 20]
        gb_2d = gaussian_blurring(
            readdump.snapshots,
            self.condition,
            ngrids,
            2.0,
        ...
        np.testing.assert_almost_equal(
            np.array([
                0.58704447, 0.56470494, 0.56153822, 0.57766662, 0.60697736,
                0.61962383, 0.59228598, 0.54755265, 0.51933923, 0.52482187
            ]),
    ...
    def test_gaussian_blurring_3d(self) -> None:
        ...
        """
        logger.info(
            f"Starting test gaussian_blurring using {self.test_file_3d}...")
        readdump = DumpReader(self.test_file_3d, ndim=3)
        readdump.read_onefile()
        ppp = np.array([1, 1, 1])
        ngrids = [20, 20, 20]
        gb_3d = gaussian_blurring(
            readdump.snapshots,
            self.condition,
            ngrids,
            2.0,
        ...
        np.testing.assert_almost_equal(
            np.array(([
                2.89425578, 2.85272144, 2.81284045, 2.77540638, 2.7436408,
                2.72687641, 2.71807427, 2.71660162, 2.72853308, 2.7535948
            ])),
    ...
    def test_gaussian_blurring_2d_vector(self) -> None:
        ...
        """
        logger.info(
            f"Starting test gaussian_blurring using {self.test_file_2d_v}...")
        ...
        gb_2d_v = gaussian_blurring(
            input_x.snapshots, condition, ngrids, 2.0, ppp)[1]
