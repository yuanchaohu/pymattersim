...
def Filon_COS(
    ...
    """
    logger.info(
        "Calculate the Fast Fourier Transformation using Filon COS method")
    ...
    results = pd.DataFrame(
        0,
        index=range(Nmax),
        columns="omega FFT".split()).astype('float64')
    for n in range(Nmax):
        omega = n * a
        ...
        if theta == 0:
            ...
            alpha = 1.0 / theta + \
                np.sin(2 * theta) / 2.0 / theta2 - 2.0 * np.sin(theta) * np.sin(theta) / theta3
            beta = 2.0 * ((1 + np.cos(theta) * np.cos(theta)) /
                          theta2 - np.sin(2 * theta) / theta3)
            gamma = 4.0 * (np.sin(theta) / theta3 - np.cos(theta) / theta2)

        C_even = 0
        for i in range(0, Nmax, 2):
            C_even += C[i] * np.cos(omega * i * dt)
        C_even -= 0.5 * (C[-1] * np.cos(omega * t[-1]) +
                         C[0] * np.cos(omega * t[0]))

        C_odd = 0
        for i in range(1, Nmax - 1, 2):
            C_odd += C[i] * np.cos(omega * i * dt)

        results.iloc[n, 1] = 2.0 * dt * (alpha * (C[-1] * np.sin(
            omega * t[-1]) - C[0] * np.sin(omega * t[0])) + beta * C_even + gamma * C_odd)
