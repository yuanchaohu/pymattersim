...
r_cut[1, 0] = 3.4
r_cut[1, 1] = 3.9
cutoffneighbors_particletype(snapshots, r_cut=r_cut, ppp=[1,1,1], fnfile='neighborlist.dat')
```
...
- Read in multiple snapshots:
  ...
  for i in range(2):	# the example dump file contains 2 snapshots
      neighbors_snapshots.append(read_neigh(f, nparticle=8100, Nmax=200))
  f.close()
  ```
...
Calling `neighbors.freud_neighbors.Voro_neighbors` first and then performing Voronoi analysis with ***[voro++](https://math.lbl.gov/voro++/)*** package for both 2D and 3D systems. Three classes of information will be output:
